      ...
      <tr key={index} id='generic-table-row' className={getHighlightsData(this.props, title, [dataArray[index].key]) ? "active" : ""}>
        <td className='custom-td' id='generic-table-key-cell'>
          {dataArray[index].checked === "notApplicable" ? null : (
            <label className='customCheckbox'>
              <input disabled={originalData[index].checked === "false" ? null : "disabled"} name={index + ".checkbox"} value={dataArray[index].checked} checked={dataArray[index].checked === "true"} type='checkbox' className='Checkbox' onChange={this.handleChange} />
        ...
        <td className='custom-td valueWrapper'>
          <div className='d-flex align-items-center'>
            <input name={index + ".value"} value={dataArray[index].type === "file" ? "" : dataArray[index].value} key={index + this.state.randomId} onChange={this.handleChange} type='text' placeholder={`Enter ${dataArray[index].key}`} className={["form-control", originalData[index].empty ? "empty-params" : ""].join(" ")} />
            {originalData[index].empty ? <div className='small mandatory-field-text'>*This field is mandatory</div> : null}
          </div>
          {dataArray[index].description?.length > 0 ? <p className='small text-muted'>{`Description: ${dataArray[index].description}`}</p> : null}
        </td>
      </tr>
    )
  }
  ...
  renderTextOrFileInput(dataArray, index) {
    ...
    return (
      <div className='position-relative fileInput'>
        <TextField {...autoCompleterDefaultProps} name={key} key={key} value={dataArray[index].key} onChange={(e) => this.handleChange(e, { name: key, value: e })} type='text' placeholder='Key' className='form-control' options={{ "{{": _.keys(this.props.environment.variables) }} />
        {title === "formData" && (
          <select
    ...
    return (
      <tr key={index} id='generic-table-row'>
        <td className='custom-td' id='generic-table-key-cell' style={{ marginLeft: "5px" }}>
          {dataArray[index].checked === "notApplicable" ? null : (
            <label className='customCheckbox'>
              <input disabled={isDashboardRoute(this.props, true) || originalData[index].checked === "false" ? null : "disabled"} name={index + ".checkbox"} value={dataArray[index].checked} checked={dataArray[index].checked === "true"} type='checkbox' className='Checkbox' onChange={this.handleChange} />
        ...
        <td className='custom-td' id='generic-table-description-cell'>
          {isDashboardRoute(this.props) ? (
            <div>
              ...
              <input disabled={isDashboardRoute(this.props) ? null : "disabled"} name={index + ".description"} value={dataArray[index].description} onChange={this.handleChange} type='text' placeholder='Description' className='form-control' />
            </div>
          ) : (
