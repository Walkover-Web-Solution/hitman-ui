...
const Endpoints = (props) => {
  ...
  function displaySingleEndpoint(endpointId) {
    ...
    const backgroundStyle = {
      backgroundImage: state.isHovered || isSelected ? `linear-gradient(to right, ${dynamicColor}, ${dynamicColor}),linear-gradient(to right, ${staticColor}, ${staticColor})` : ''
    }
          ...
          <div className='sidebar-toggle d-flex justify-content-between mt-1'>
            <button>
              <div className={`side-bar d-flex align-items-center rounded mr-2 ${isSelected ? 'Selected' : ''}`} style={backgroundStyle}
                ...
                <button
                  tabIndex={-1}
                  onClick={() => { handleDisplay(props.endpoints[endpointId], props.endpointId, props.collection_id, true) }}
                  onDoubleClick={() => handleDisplay(props.endpoints[endpointId], props.endpointId, props.collection_id, false)}
                >
  ...
  const handleHover = (isHovered) => {
    setState((prev)=> {return {...prev, isHovered }});
  };
  ...
  return displayUserEndpoints(props?.endpointId)
}
