...
const Endpoints = (props) => {
  ...
  const displayEndpointName = (endpointId) => {
    const isSelected =
      isOnPublishedPage() && sessionStorage.getItem('currentPublishIdToShow') === endpointId
        ? 'selected'
        : isDashboardRoute({ location, navigate }) && params.endpointId === endpointId
          ? 'selected'
          : ''
    return (
      <div className={`sidebar-accordion-item ${isSelected ? 'Selected' : ''}`}>
        {endpoints[endpointId]?.protocolType === 1 && <div className={`api-label ${endpoints[endpointId].requestType} request-type-bgcolor ${!isOnPublishedPage() ? 'in-api-label' : ''}`}>
  ...
  const displayEndpointOptions = (endpointId) => (
    <div className='sidebar-item-action'>
      ...
      <div className='dropdown-menu dropdown-menu-right'>
        <div className='dropdown-item d-flex' onClick={() => {
          setShowEndpointForm((prev) => ({ ...prev, edit: true }))
          setSelectedEndpoint(endpoints[endpointId])
        }}>
          <RenamedItem /> Rename
        </div>
        <div className='dropdown-item d-flex' onClick={() => dispatch(duplicateEndpoint(endpoints[endpointId]))}>
          <Duplicate /> Duplicate
        </div>
        <div className='dropdown-item text-danger d-flex' onClick={() => {
          setShowEndpointForm((prev) => ({ ...prev, delete: true }))
          setSelectedEndpoint(endpoints[endpointId])
        }}>
          <DeleteIcon /> Delete
        </div>
            ...
            <button>
              ...
              >
                ...
                <div className='d-flex align-items-center'>
                  {isDashboardRoute({ navigate, location }, true) &&
