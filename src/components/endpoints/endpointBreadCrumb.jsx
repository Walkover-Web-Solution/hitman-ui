  ...
  switchProtocolTypeDropdown() {
    return (
      <div className='dropdown'>
        <button className='protocol-selected-type mr-2' id='dropdownMenuButton' data-toggle='dropdown' aria-haspopup='true' aria-expanded='false'>
      ...
      <div className='endpoint-header'>
        <div className='panel-endpoint-name-container'>
          <div className='page-title-name d-flex align-items-center'>
            ...
            {this.props?.match?.params?.endpointId != "new" && this.props?.endpointContent?.protocolType === 1 && this.state?.protocols?.[0]?.icon && <button className='protocol-selected-type cursor-text mr-2'>{this.state.protocols?.[0]?.icon}</button>}
            {this.props?.match?.params?.endpointId != "new" && this.props?.endpointContent?.protocolType === 2 && this.state?.protocols?.[1]?.icon && <button className='protocol-selected-type cursor-text mr-2'>{this.state.protocols?.[1]?.icon}</button>}
            <input
              name='enpoint-title'
              ...
              value={this.props?.isEndpoint ? this.props?.pages?.[this.props?.match?.params?.endpointId]?.name || this.props?.history?.[this.props?.match?.params?.historyId]?.endpoint?.name || this.props?.endpointContent?.data?.name : this.props?.pages?.[this.props?.match?.params?.pageId]?.name}
            />
          </div>
          {this.props.location.pathname.split("/")[5] !== "new" && (
            <div className='d-flex bread-crumb-wrapper align-items-center text-nowrap'>
              ...
              {this.props?.endpoints[this.props.currentEndpointId]?.isPublished && <div className='api-label POST request-type-bgcolor ml-2'> Live </div>}
              {this.props.pages?.[this.props?.match?.params?.pageId]?.isPublished && <div className='api-label POST request-type-bgcolor ml-2'> Live </div>}
            </div>
          )}
        </div>
      </div>
