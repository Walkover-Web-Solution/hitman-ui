...
class BodyContainer extends Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    } else if (this.props?.body?.type === 'none' ) {
      this.updateStateBasedOnBodyType('none', null)
    }
  ...
  updateStateBasedOnBodyType(bodyType, value) {
    if(this.showRawBodyType == true && ['multipart/form-data', 'application/x-www-form-urlencoded', 'none'].includes(this.props?.body?.type)){
      this.showRawBodyType = false
    }

    let bodyTypeToUpdate = bodyType == 'raw' ? 'raw' : (bodyType == 'none') ? null :this.props?.body?.type

    if (_.isEqual(value, this.state.data[bodyType]) && this.state.selectedBodyType == bodyTypeToUpdate) return // No need to update if the value hasn't changed
    ...
    this.setState((prevState) => ({
      ...
      selectedBodyType: bodyTypeToUpdate
    }))
    if (bodyType == 'raw') {
