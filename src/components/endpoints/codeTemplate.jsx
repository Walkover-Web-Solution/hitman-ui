...
const CodeTemplate = (props) => {
  ...
  const OutlineArrowRef = useRef();
  const pubCodesRef = useRef();
  const iconNoneRef = useRef();
  ...
  return (
    <div className={props.match.params.endpointId ? "show-curl-endpoint pubCodeWrapper" : "pubCodeWrapper"} style={{ backgroundColor: hexToRgb(theme, '0.04') }}>
        ...
        <Col id='code-window-sidebar' xs={12} className=''>
          ...
          <div className='select-code-wrapper d-flex align-items-center mb-3 img'>
            {primaryLanguages.map((key) => (
              <button
                key={key}
                className={`${props.match.params.endpointId ? "btn btn-outline-dark" : ""} ${getClassForLanguages(key)}`}
                onClick={() => {
