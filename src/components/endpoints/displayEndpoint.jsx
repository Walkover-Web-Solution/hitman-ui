...
const mapDispatchToProps = (dispatch, ownProps) => {
  return {
    add_endpoint: (newEndpoint, rootParentID, callback, props) => dispatch(addEndpoint(ownProps.history, newEndpoint, rootParentID, callback, props)),
...
const getEndpointContent = async (props) => {
  ...
  let endpointId = isUserOnPublishedPage ? currentIdToShow : props?.match?.params.endpointId !== "new" ? props?.match?.params?.endpointId : props?.activeTabId
...
const withQuery = (WrappedComponent) => {
  return (props) => {
    ...
    let endpointId = isOnPublishedPage() ? currentIdToShow : props?.match?.params?.endpointId !== "new" ? props?.match?.params?.endpointId : props?.activeTabId
...
class DisplayEndpoint extends Component {
  ...
  async componentDidMount() {
    ...
    this.endpointId = this.props.endpointId ? this.props.endpointId : isDashboardRoute(this.props) ? this.props.location.pathname.split("/")[5] : this.props.location.pathname.split("/")[4]
          ...
          pathVariables.push({
            ...
            value: this.props.endpointContent.pathVariables[counter] // TODO :: correct this index and assign correct value
              ? this.props.endpointContent.pathVariables[counter].key === pathVariableKeyWithoutColon
                ...
                : ""
              : "",
            description: this.props.endpointContent.pathVariables[counter] ? (this.props.endpointContent.pathVariables[counter].key === pathVariableKeyWithoutColon ? this.props.endpointContent.pathVariables[counter].description : "") : ""
          })
          counter++
        }
      }
  ...
  displayPublicSampleResponse() {
    if (this.props?.endpointContent?.sampleResponseArray?.length) {
      return (
        <div className='mt-3'>
          <PublicSampleResponse highlights={this.props?.highlights} sample_response_array={this.props?.endpointContent?.sampleResponseArray} publicCollectionTheme={this.props?.publicCollectionTheme} />
        </div>
      )
    }
  }
          ...
          <div className='d-flex justify-content-between align-items-center'>
            <ul className='nav nav-tabs respTabsListing' id='myTab' role='tablist'>
              <li className='nav-item'>
                <a className='nav-link active' id='pills-response-tab' data-toggle='pill' href={this.isDashboardAndTestingView() ? `#response-${this.props.tab.id}` : "#response"} role='tab' aria-controls={this.isDashboardAndTestingView() ? `response-${this.props.tab.id}` : "response"} aria-selected='true'>
                  Response
                </a>
              </li>
              {getCurrentUser() && (
                <li className='nav-item'>
                  <a className='nav-link' id='pills-sample-tab' data-toggle='pill' href={this.isDashboardAndTestingView() ? `#sample-${this.props.tab.id}` : "#sample"} role='tab' aria-controls={this.isDashboardAndTestingView() ? `sample-${this.props.tab.id}` : "sample"} aria-selected='false'>
                    Sample Response
                  </a>
                </li>
              )}
  ...
  renderTiptapEditor(item, index) {
    return <Tiptap initial={item.data} onChange={(e) => this.debouncedSave(index, e)} match={this.props.match} isInlineEditor disabled={!isDashboardRoute(this.props)} key={`${item.type}-${index}`} />
  }
  ...
  renderPublicHeaders() {
    return this.props?.endpointContent?.originalHeaders?.length > 0 && <GenericTable {...this.props} title='Headers' dataArray={this.props?.endpointContent?.originalHeaders} props_from_parent={this.propsFromChild.bind(this)} original_data={[...this.props?.endpointContent?.originalHeaders]} currentView={this.props?.endpointContent?.currentView} />
  }
  ...
  renderPublicParams() {
    return (
      this.props?.endpointContent?.originalParams?.length > 0 && (
        <div>
          <GenericTable {...this.props} title='Params' dataArray={this.props?.endpointContent?.originalParams || []} props_from_parent={this.propsFromChild.bind(this)} original_data={this.props?.endpointContent?.originalParams} currentView={this.props?.endpointContent?.currentView} />
        </div>
      )
    )
  }
  ...
  renderPublicPathVariables() {
    return (
      ...
      this.props?.endpointContent?.pathVariables?.length !== 0 && (
        <div>
          <GenericTable {...this.props} title='Path Variables' dataArray={this.props?.endpointContent?.pathVariables} props_from_parent={this.propsFromChild.bind(this)} original_data={this.props?.endpointContent?.pathVariables} currentView={this.props?.endpointContent?.currentView} />
        </div>
      )
    )
  }
          ...
          <div className='input-group'>
            {this.checkProtocolType(1) && (
              <div className='input-group-prepend'>
                <span className={`api-label api-label-lg input-group-text ${this.props?.endpointContent?.data?.method}`}>{this.props?.endpointContent?.data?.method}</span>
              </div>
            )}
            <HostContainer
              {...this.props}
      ...
      <div className='input-group-prepend'>
        {this.checkProtocolType(1) && (
          <div className='dropdown'>
            <button className={`api-label ${this.props?.endpointContent?.data?.method} dropdown-toggle`} type='button' id='dropdownMenuButton' data-toggle={"dropdown"} aria-haspopup={"true"} aria-expanded={"false"} disabled={isDashboardRoute(this.props) ? null : true}>
  ...
  renderDocViewOperations() {
    ...
    if (isDashboardRoute(this.props) && this.props?.endpointContent?.currentView === "doc" && endpointss) {
      ...
      return (
        <div>
          {isStatePending(endpointId, endpointss) && isAdmin() && <ApproveRejectEntity {...this.props} entity={endpointss} entityId={endpointId} entityName='endpoint' />}
          <button id='api_save_btn' className={this.state.saveLoader ? "ml-2 btn btn-outline orange buttonLoader btn-sm fs-4" : "ml-2 btn btn-outline orange btn-sm fs-4"} type='button' onClick={() => this.handleSave()}>
            {isPublicEndpoint ? "Save Draft" : "Save"}
          </button>
          {isAdmin() && !isStatePending(endpointId, endpointss) && <span> {approvedOrRejected ? this.renderInOverlay(this.renderPublishEndpoint.bind(this), endpointId) : this.renderPublishEndpoint(endpointId, endpointss)}</span>}
          {isAdmin() && isPublicEndpoint && <span> {isStateApproved(endpointId, endpointss) ? this.renderInOverlay(this.renderUnPublishEndpoint.bind(this), endpointId) : this.renderUnPublishEndpoint(endpointId, endpointss)}</span>}
          {!isAdmin() && (
            <button className={"ml-2 " + (isStateDraft(endpointId, endpointss) ? "btn btn-outline orange" : "btn text-link")} type='button' onClick={() => (isStateDraft(endpointId, endpointss) ? this.handlePublicEndpointState(this.props.pages[endpointId]) : null)}>
  ...
  renderUnPublishEndpoint(endpointId, endpointss) {
    return <UnPublishEntityButton entity={endpointss} entityId={endpointId} onUnpublish={() => this.handleRemovePublicEndpoint(endpointId)} entityName='Endpoint' />
  }

  renderPublishEndpoint(endpointId, endpoints) {
    return <PublishEntityButton entity={endpoints} entityId={endpointId} open_publish_confirmation_modal={() => this.setState({ openPublishConfirmationModal: true })} entityName='Endpoint' />
  }

  renderPublishConfirmationModal() {
    return this.state.openPublishConfirmationModal && <ConfirmationModal show={this.state.openPublishConfirmationModal} onHide={() => this.setState({ openPublishConfirmationModal: false })} proceed_button_callback={this.handleApproveEndpointRequest.bind(this)} title={msgText.publishEndpoint} submitButton='Publish' rejectButton='Discard' />
  }

  renderUnPublishConfirmationModal() {
    return this.state.openUnPublishConfirmationModal && <ConfirmationModal show={this.state.openUnPublishConfirmationModal} onHide={() => this.setState({ openUnPublishConfirmationModal: false })} proceed_button_callback={this.handleRejectEndpointRequest.bind(this)} title={msgText.unpublishEndpoint} submitButton='UnPublish' rejectButton='Discard' />
  }
        ...
        {this.isDashboardAndTestingView() ? (
          this.props.location.pathname.split("/")[5] !== "new" ? (
            <Dropdown as={ButtonGroup}>
              <button id='api_save_btn' className={this.state.saveLoader ? "btn btn-outline orange buttonLoader btn-sm d-flex align-items-center" : "btn btn-outline orange btn-sm d-flex align-items-center"} type='button' onClick={() => this.handleSave()}>
            ...
            <button className={this.state.saveLoader ? "btn btn-outline orange buttonLoader btn-sm fs-4 d-flex align-items-center" : "btn btn-outline orange btn-sm fs-4 d-flex align-items-center"} type='button' id='save-endpoint-button' onClick={() => this.handleSave()}>
              <LiaSaveSolid className='save-icon mr-1' size={16} />
              <span>Save</span>
            </button>
          )
  ...
  render() {
    ...
    return (isDashboardRoute(this.props) && this.props?.endpointContent?.currentView) || !isDashboardRoute(this.props) || !isSavedEndpoint(this.props) ? (
      <div ref={this.myRef} className={!this.isNotDashboardOrDocView() ? "" : codeEditorVisibility ? "mainContentWrapperPublic hideCodeEditor" : "mainContentWrapperPublic "}>
          ...
          <div className={`innerContainer ${"response-bottom"}`}>
            <div className={`hm-endpoint-container mid-part endpoint-container ${this.props?.endpointContent?.currentView === "doc" ? "doc-fix-width" : ""}`}>
                  ...
                  {isDashboardRoute(this.props) && (
                    <div className='hm-panel'>
                      ...
                      <div className='position-relative top-part'>
                        ...
                        {this.isDashboardAndTestingView() && <DisplayDescription {...this.props} endpoint={this.state.endpoint} data={this.state.data} old_description={this.state.oldDescription} props_from_parent={this.propsFromDescription.bind(this)} alterEndpointName={(name) => this.alterEndpointName(name)} />}
              ...
              <div className={"clear-both " + (this.props?.endpointContent?.currentView === "doc" ? "doc-view" : "testing-view")}>
                        ...
                        <>
                          ...
                          {!isDashboardRoute(this.props) && (
                            <div className='request-button'>
                              <button className={this.state.loader ? "btn custom-theme-btn btn-lg buttonLoader" : "btn btn-lg custom-theme-btn px-md-4 px-3"} style={{ background: theme }} type='submit' id='send-request-button' onClick={() => this.handleSend()}>
                                TRY
                              </button>
                            </div>
                          )}
                  ...
                  {this.isDashboardAndTestingView() && (
                    <div className='endpoint-url-container'>
                      ...
                      <div className='d-flex uriContainerWrapper'>
                        <button className={this.state.loader ? "btn btn-primary buttonLoader" : "btn btn-primary"} type='submit' id='api-send-button' onClick={() => this.handleSend()} disabled={this.state.loader}>
                    ...
                    <div className='main-table-wrapper'>
                        ...
                        <div className='d-flex justify-content-between align-items-center'>
                          <div className='headers-params-wrapper custom-tabs'>
                            <ul className='nav nav-tabs' id='pills-tab' role='tablist'>
                              {this.checkProtocolType(1) && (
                                <li className='nav-item'>
                                  <a className={this.setAuthorizationTab ? "nav-link " : "nav-link active"} id='pills-params-tab' data-toggle='pill' href={`#params-${this.props.tab.id}`} role='tab' aria-controls={`params-${this.props.tab.id}`} aria-selected={this.setAuthorizationTab ? "false" : "true"}>
                                    Params
                                  </a>
                                </li>
                              )}
                              {this.checkProtocolType(2) && (
                                <li className='nav-item'>
                                  <a className={this.setAuthorizationTab ? "nav-link " : "nav-link active"} id='pills-query-tab' data-toggle='pill' href={`#query-${this.props.tab.id}`} role='tab' aria-controls={`query-${this.props.tab.id}`} aria-selected='false'>
                                    Query
                                  </a>
                                </li>
                              )}
                              <li className='nav-item'>
                                <a className={this.setAuthorizationTab ? "nav-link active" : "nav-link "} id='pills-authorization-tab' data-toggle='pill' href={`#authorization-${this.props.tab.id}`} role='tab' aria-controls={`authorization-${this.props.tab.id}`} aria-selected={this.setAuthorizationTab ? "true" : "false"}>
                                  Authorization
                                </a>
                              </li>
                              <li className='nav-item'>
                                <a className='nav-link' id='pills-headers-tab' data-toggle='pill' href={`#headers-${this.props.tab.id}`} role='tab' aria-controls={`headers-${this.props.tab.id}`} aria-selected='false'>
                                  Headers
                                </a>
                              </li>
                              {this.checkProtocolType(1) && (
                                <li className='nav-item'>
                                  <a className='nav-link' id='pills-body-tab' data-toggle='pill' href={`#body-${this.props.tab.id}`} role='tab' aria-controls={`body-${this.props.tab.id}`} aria-selected='false'>
                                    Body
                                  </a>
                                </li>
                              )}
                              <li className='nav-item'>
                                <a className='nav-link' id='pills-pre-script-tab' data-toggle='pill' href={`#pre-script-${this.props.tab.id}`} role='tab' aria-controls={`pre-script-${this.props.tab.id}`} aria-selected='false'>
                                  Pre-Script
                                </a>
                              </li>
                              <li className='nav-item'>
                                <a className='nav-link' id='pills-post-script-tab' data-toggle='pill' href={`#post-script-${this.props.tab.id}`} role='tab' aria-controls={`post-script-${this.props.tab.id}`} aria-selected='false'>
                                  Post-Script
                                </a>
                              </li>
                              {this.checkProtocolType(1) && (
                      ...
                      {this.isDashboardAndTestingView() ? (
                        <div className='tab-content' id='pills-tabContent'>
                          {this.checkProtocolType(1) && (
                            <div className={this.setAuthorizationTab ? "tab-pane fade" : "tab-pane fade show active"} id={`params-${this.props.tab.id}`} role='tabpanel' aria-labelledby='pills-params-tab'>
                              {this.renderParams()}
                              <div>{this.renderPathVariables()}</div>
                            </div>
                          )}
                          {this.checkProtocolType(2) && (
                            <div className={this.setAuthorizationTab ? "tab-pane fade" : "tab-pane fade show active"} id={`query-${this.props.tab.id}`} role='tabpanel' aria-labelledby='pills-query-tab'>
                          ...
                          <div className={this.setAuthorizationTab ? "tab-pane fade show active" : "tab-pane fade "} id={`authorization-${this.props.tab.id}`} role='tabpanel' aria-labelledby='pills-authorization-tab'>
                            <div>
                              <Authorization {...this.props} set_authorization_headers={this.setHeaders.bind(this)} set_authoriztaion_params={this.setParams.bind(this)} set_authoriztaion_type={this.setAuthType.bind(this)} handleSaveEndpoint={this.handleSave.bind(this)} />
                            </div>
                          </div>
                ...
                {!this.isDashboardAndTestingView() && isDashboardRoute(this.props) && <div className='doc-options d-flex align-items-center'>{this.renderDocViewOptions()}</div>}
              </div>
