...
class DisplayEndpoint extends Component {
	...
	findHost() {
		...
		} else if (Object.keys(this.state.endpoint).length && this.state.title === 'update endpoint') {
	...
	finalUrl(api) {
		...
		for (let i = 0; i < variables.length; i++) {
			if (!this.props.environment.variables[variables[i]]) {
				...
				api = api.replace(`{{${variables[i]}}}`, this.props.environment.variables[variables[i]].currentValue);
			} else if (this.props.environment.variables[variables[i]].initialValue) {
				api = api.replace(`{{${variables[i]}}}`, this.props.environment.variables[variables[i]].initialValue);
			} else {
				api = api.replace(`{{${variables[i]}}}`, '');
			}
		}
	...
	handleSave = async (e) => {
		...
		if (endpoint.name == '' || endpoint.uri == '') toast.error('Please Enter all the fields');
