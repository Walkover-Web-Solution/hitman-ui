...
class Environments extends Component {
	...
	async handleAdd(newEnvironment) {
		...
		this.setState({ environments });
		try {
			const { data: environment } = await environmentService.saveEnvironment(newEnvironment);
	...
	async handleUpdateEnvironment(updatedEnvironment) {
		if (JSON.stringify(this.state.environments[this.state.environmentId]) !== JSON.stringify(updatedEnvironment)) {
			const originalEnvironment = jQuery.extend(true, {}, this.state.environment);
			const environments = { ...this.state.environments };
			...
			try {
				...
				delete body.id;
				const { data: environment } = await environmentService.updateEnvironment(updatedEnvironment.id, body);
