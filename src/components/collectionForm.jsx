...
class CollectionForm extends Form {
	...
	render() {
		...
		if (this.props.selectedcollection && this.state.editCollection) {
			...
			this.state.data.keyword = this.props.selectedcollection.keyword.split(',')[0];
			this.state.data.keyword1 = this.props.selectedcollection.keyword.split(',')[1];
			this.state.data.keyword2 = this.props.selectedcollection.keyword.split(',')[2];
			...
			<Modal {...this.props} size="lg" aria-labelledby="contained-modal-title-vcenter" centered>
				...
				<Modal.Body>
					<form onSubmit={this.handleSubmit}>
						...
						<div className="row">
							<div className="col">{this.renderInput('keyword', 'Keyword 1*')}</div>
							<div className="col">{this.renderInput('keyword1', 'Keyword 2')}</div>
							<div className="col">{this.renderInput('keyword2', 'Keyword 3')}</div>
						</div>
