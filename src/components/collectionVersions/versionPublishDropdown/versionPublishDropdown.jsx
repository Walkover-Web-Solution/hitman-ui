...
export default function VersionPublishDropdown(props) {
  ...
  return (
    <DropdownButton className='' id='dropdown-basic-button' onClick={(event) => event.stopPropagation()} title={<span className='dropdown-title'>{pages?.[props.rootParentId]?.child?.length === 1 ? defaultVersion?.name : selectedVersion?.name}</span>}>
