...
class CollectionParentPages extends Component {
  ...
  setSelectedVersionId (id, value) {
    if (id && this.state.selectedParentPageIds[id] !== value) {
      this.setState({ selectedParentPageIds: { ...this.state.selectedParentPageIds, [id]: value } })
    }
  }
  ...
  openAddVersionPageForm (version) {
    ...
    this.setState({
      showPageForm,
      pageFormName: 'Add New Version Page',
      selectedPage: version
    })
  }
  ...
  propsFromParentPage (pageIds, title) {
    ...
    if (title === 'pages') {
      ...
      if (pageIds !== null) {
        for (let i = 0; i < pageIds.length; i++) {
          this.filteredCollectionPages[pageIds[i]] = this.props.pages[
            pageIds[i]
          ]
          this.eventkey[pageIds[i]] = '0'
        }
  ...
  filterPages () {
    if (
      ...
      if (pageIds.length !== 0) {
        for (let i = 0; i < pageIds.length; i++) {
          this.filteredOnlyParentPages[pageIds[i]] = this.props.pages[
            pageIds[i]
          ]
          if (
            !this.eventkey[pageIds[i]] ||
            this.eventkey[pageIds[i]] !== '0'
          ) {
        ...
        ? (
          <div className={['hm-sidebar-outer-block'].join(' ')} key={pageId}>
            ...
            >
              ...
              >
                <div className='d-flex align-items-center cl-name' onClick={() => { this.toggleParentPageIds(pageId) }}>
  ...
  getVersionsCount (filteredPages) {
    const versionsCount = Object.keys(filteredPages || {}).filter(
      (pageId) =>
        filteredPages[pageId].collectionId ===
      ...
      <>
          ...
          )}
        {
          !isDashboardRoute(this.props, true)
            ? (
              <>
                <div className={this.filteredPages && Object.keys(this.filteredPages).length > 1 ? this.state.enableSearch ? 'versionWrapper versionsAvailable d-flex enableSearch' : 'versionWrapper versionsAvailable d-flex' : 'versionWrapper'}>
