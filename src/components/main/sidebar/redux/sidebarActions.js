  ...
  if (!_.isEmpty(itemNode)) {
    ...
    if (isExpandable) {
      if (!expanded || isOpen) {
        ...
        store.dispatch({ type: sidebarActionTypes.COLLAPSE_ITEM })
      }
    }
  }
}
...
const duplicateEntity = (nodeAddress) => {
  ...
  switch (type) {
    case 'collections':
      ...
      break
    case 'versions':
      store.dispatch(duplicateVersion(versions[id]))
      break
    case 'groups':
      store.dispatch(duplicateGroup(groups[id]))
      break
    case 'pages':
      store.dispatch(duplicatePage(pages[id]))
      break
    case 'endpoints':
      store.dispatch(duplicateEndpoint(endpoints[id]))
      break
    default:
      break
  }
...
const deleteEntity = (nodeAddress, props) => {
  ...
  switch (type) {
    case 'collections':
      collections[id].importedFromMarketPlace
        ? store.dispatch(removePublicCollection(collections[id], props))
        : store.dispatch(deleteCollection(collections[id], props))
      break
    case 'versions':
      store.dispatch(deleteVersion(versions[id], props))
      break
    case 'groups':
      store.dispatch(deleteGroup(groups[id], props))
      break
    case 'pages':
      store.dispatch(deletePage(pages[id], props))
      break
    case 'endpoints':
      store.dispatch(deleteEndpoint(endpoints[id], props))
      break
    default:
      break
  }
