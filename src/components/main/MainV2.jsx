...
import {
  fetchCollections,
  fetchCollectionsFromIdb,
} from "../collections/redux/collectionsActions";
import {
  fetchAllVersions,
  fetchAllVersionsFromIdb,
...
import {
  fetchAllCookies,
  fetchAllCookiesFromLocalStorage,
} from "../cookies/redux/cookiesActions";
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    fetch_all_cookies_from_local: () =>
      dispatch(fetchAllCookiesFromLocalStorage()),
    fetch_endpoint: (endpointId) => dispatch(fetchEndpoint(endpointId)),
    fetch_page: (pageId) => dispatch(fetchPage(pageId)),
    udpate_collection_and_pages_in_sidebar: (orgId) =>
      dispatch(addCollectionAndPagesInSideBar(orgId)),
  };
};
...
class MainV2 extends Component {
  ...
  async isIdbUpdated() {
    ...
    if (
      timestampIdb &&
      moment(timestampIdb).isValid() &&
      moment(timestampIdb).diff(moment(timestampBackend)) >= 0
    ) {
      fetchFromIdb = true;
    } else {
      fetchFromIdb = false;
  ...
  showCollectionDashboard() {
    ...
    if (
      (temp && temp[orgId]) ||
      collectionLength > 0 ||
      !this.state.showAddCollectionPage
    ) {
      return false;
    } else {
      return true;
    ...
    return (
      <>
        ...
        <div className="no-collection h-100 d-flex flex-d-col justify-content-center align-items-center flex-wrap">
          ...
          <button
            onClick={() => this.setState({ showAddCollectionModal: true })}
          ...
          <div>
            {!isDesktop && (
              <div className="mobile-warning">
                Looks like you have opened it on a mobile device. It looks
                better on a desktop device.
              </div>
            )}
            {
              <div className="custom-main-container">
                ...
                <OnlineSatus
                  ...
                  isIdbUpdated={this.isIdbUpdated.bind(this)}
                />
