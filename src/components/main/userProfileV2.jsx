...
export class UserProfileV2 extends React.Component {
    ...
    async switchOrg (orgId) {
      try {
        ...
        if (response.ok && responseData.success) { // Assuming the API returns a 'success' field to indicate a successful switch
          this.redirectToDashboard(orgId)
        } else {
          // Handle error, perhaps show an error message to the user
          console.error('Error switching organization:', responseData.message)
        }
      } catch (error) {
