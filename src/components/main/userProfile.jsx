...
import OpenApiForm1 from '../openApi/openApiForm1'
import CollectionForm from '../collections/collectionForm1'
import { MdSwitchLeft } from 'react-icons/md'
import { FiUser } from 'react-icons/fi'
import OpenApiForm1Jsx from '../openApi/openApiForm1'
...
const UserProfile = (props) => {
  ...
  const handleNewOrgClick = async () => {
    toggleModal()
    const tabIdsToClose = props.tabs.tabsOrder
    if (tabIdsToClose.length === 1 || tabIdsToClose.length === 0) {
      setModalForTabs(false)
      removeFromLocalStorage(tabIdsToClose)
      props.close_all_tabs(tabIdsToClose)
      props.remove_history(props.historySnapshot)
  ...
  const renderOrgName = () => {
    return (
      <div>
        ...
        {/* Example conditional rendering of a notification badge */}
        {/* {notificationCount > 0 && <div className='user-notification-badge'>{notificationCount}</div>} */}
      </div>
    )
  }
    ...
    return (
      <div className='menu-trigger-box d-flex align-items-center justify-content-between w-100'>
        ...
        >
          ...
          {renderOrgName()}
          {/* Uncomment if notification logic is implemented
          {notificationCount > 0 &&
        ...
        <div className='add-button d-flex align-items-center'>
          ...
          <OpenApiForm1 show={showImportModal} onHide={handleImportClick} />
          <CollectionForm show={showNewCollectionModal} title='Add new Collection' onHide={handleAddNewClick} />
        </div>
      </div>
    )
  }
    ...
    return (
      <div className='profile-details border-bottom plr-3 pb-1 d-flex align-items-center py-1' onClick={() => {}}>
        ...
        <div className='profile-details-user-name'>
          {/* Uncomment and use if you have a 'name' to display as well */}
  ...
  const renderOrgListDropdown = () => {
    const organizations = props.organizationList || []
  ...
  const handleTrashClick = () => {
    ...
    props.history.push(`/orgs/${currentOrgId}/trash`)
  }
  ...
  const handleTabsandHistory = async (value) => {
    const tabIdsToClose = props.tabs.tabsOrder
    const history = props.historySnapshot

    if (value === 'yes') {
      props.close_all_tabs(tabIdsToClose)
      removeFromLocalStorage(tabIdsToClose)
      props.remove_history(history)
      if (switchOrCreate) {
        createOrg(orgName)
      } else {
    ...
    <>
      <div className='profile-menu'>
        <Dropdown className='d-flex align-items-center'>
          <Dropdown.Toggle
            as={forwardRef(({ children, onClick }, ref) => renderAvatarWithOrg(onClick, ref))}
