...
class Collections extends Component {
	...
	async fetchVersions(collections) {
		...
		for (let i = 0; i < collectionIds.length; i++) {
			const { data: versions1 } = await collectionVersionsService.getCollectionVersions(collectionIds[i]);
	...
	async fetchEndpoints(groups) {
		...
		for (let i = 0; i < groupIds.length; i++) {
			let { data: newEndpoints } = await endpointService.getEndpoints(groupIds[i]);
	...
	async handleDelete(collection) {
		...
		const collectionIds = this.state.collectionIds.filter((cId) => cId !== collection.id);
	...
	async handleDeleteVersion(deletedCollectionVersionId) {
		...
		const versionIds = this.state.versionIds.filter((vId) => vId !== deletedCollectionVersionId.toString());
	...
	async handleDeleteGroup(deletedGroupId) {
		...
		const groupIds = this.state.groupIds.filter((gId) => gId !== deletedGroupId.toString());
		this.setState({ groups, groupIds });
		try {
	...
	async handleUpdateGroup(editedGroup) {
		...
		try {
			...
			delete body.id;
			const { data: group } = await groupsService.updateGroup(editedGroup.id, body);
	...
	async handleDeletePage(deletedPageId) {
		...
		const pageIds = this.state.pageIds.filter((pId) => pId !== deletedPageId.toString());
		this.setState({ pages, pageIds });
		try {
	...
	onDragOver = (e, index) => {
		...
		let collectionIds = this.state.collectionIds.filter((item) => item !== this.draggedItem);
	...
	render() {
		...
		if (location.title === 'update Endpoint') {
			...
			this.updateEndpoint(endpointId, groupId, endpoint, this.props.location.versions);
		}
								...
								>
									...
									>
										<Dropdown.Item
											...
											onClick={() => {
												this.props.history.push({
													...
													edited_collection: this.state.collections[collectionId]
												});
											}}
										>
											Edit
										...
										<Dropdown.Item
											...
											onClick={() => {
												if (
													...
													this.handleDelete(this.state.collections[collectionId]);
											}}
										>
											Delete
										</Dropdown.Item>
										...
										<Dropdown.Item
											eventKey="3"
											onClick={() =>
												this.handleDuplicateCollection(this.state.collections[collectionId])}
										>
											Duplicate
										</Dropdown.Item>
									</DropdownButton>
