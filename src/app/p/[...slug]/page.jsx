...
export async function headerFooter({ params, searchParams, customDomain }) {
  let response,
    queryParamsString = getQueryParamsString({ params, searchParams, customDomain })
  try {
...
export default async function Page({ params, searchParams, customDomain }) {
  ...
  return (
    <div className='w-100vw h-100vh hm-main-wrapper'>
      ...
      <div className='d-flex m-auto main-public-page-container px-5 py-2 min-vh-100 max-width-container'>
      ...
      {content?.defaultFooter !== '' && (
        <div className='preview-content mx-auto' dangerouslySetInnerHTML={{ __html: content?.defaultFooter ?? '' }} />
      )}
    </div>
  )
}
